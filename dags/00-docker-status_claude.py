...
with DAG('05_three_tasks',
    ...
    # This way, the volume persists across containers and no host path resolution needed
    
    write = DockerOperator(
        task_id='write_file',
        image='alpine:latest',
        command=['sh', '-c', 'echo "Hello from first" > /data/message.txt && cat /data/message.txt'],
        mounts=[Mount(source='commodity-data',
                      target='/data',
                      type='volume')],  # Use 'volume' instead of 'bind'
    )
    ...
    read = DockerOperator(
        ...
        mounts=[Mount(source='commodity-data',  # same volume
                  target='/data',
                  type='volume')],
    )
